...
The Lean ZEPs PR should be submitted in the [zarr-specs] repository with the name
`<zep-<n>: <title>.rst` under [/zarr-specs/docs/specs/...], where `<n>` is an
appropriately assigned four-digit number.
...
the [zarr-specs] repository with the name `zep-<n>: <title>.rst` where `<n>` is
an appropriately assigned four-digit number (e.g., zep-0000.rst). The draft ZEP

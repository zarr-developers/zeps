...
website, but are still work in progress and not part of this ZEP:

* [Data types](https://zarr-specs.readthedocs.io/en/latest/v3/data-types.html)
  (not part of a ZEP at the time of writing)

* Storage transformers - [Sharding storage
  transformer](https://zarr-specs.readthedocs.io/en/latest/v3/array-storage-transformers/sharding/v1.0.html)
...
In Zarr v3 this becomes "/foo/bar/zarr.json". This change means that
all the names and types (array or group) of children of the "/foo"
group can be discovered and listed via a single storage operation to
list the objects with prefix "/foo" and delimiter "/".
...
The following projects contain implementations of Zarr Version 3 protocol:

- zarrita: <https://github.com/alimanfoo/zarrita>

> Zarrita is a minimal, exploratory implementation of the Zarr version
3.0 core protocol. This is a technical spike only, not for production use. It is
not up to date to the current specification
...
> `zarr-python` implements a slightly outdated version of Zarr v3, updating to
the current spec is tracked in <https://github.com/zarr-developers/zarr-python/issues/1290>.

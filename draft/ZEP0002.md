...
In many cases it becomes inefficient or impractical to store a large number of chunks directly as
files or objects due to the design constraints of the underlying storage.
...
The dataset has a shape of `(25000, 18000, 6000)` and a dtype of `uint8`. This makes for a total size of 2.4TB. For efficient reads and fast streaming of the data, e.g. to a browser-based interface, chunk sizes should be of max. `(64, 64, 64)`. For this dataset that would result in 10.3M chunks, and withouth sharding also to 10.3M files. Considering even many of these datasets, the number of files becomes impractical. With sharding, one could use shards of size `(2048, 2048, 2048)` which would each contain `(32, 32, 32)` chunks. That would reduce the number of files to ~300, while maintaining a practical write granularity.

Sharding could be implemented directly in the storage layer, but to allow interoperability between different stores and Zarr implementations, it is important to be able to consistently adress array data within a store. Therefore, standardizing the sharding format across different stores is crucial for interoperability, which is possible by implementing sharding as a storage transformer.
...
All of those strategies only shard the array-data, both as metadata must first be read without
sharding, so that sharding itself can be configured in the metadata, and also since the relevant
parts of the data is the array data itself.
...
Comparing strategy 3 and 2, in opposite to the load balancing, it is more important to co-locate
data that us usually accessed together or in short succession. This is optimized by combining
multiple chunks that are close to each other, what is supported best by strategy 2, using
...
chunking and indexing is done. Therefore could be implemented directly in the storage layer,
...
to consistently adress array data across different stores and ensures compatibility
...
1. Using a chunk-size and shard-size of 1 for the sliced dimension effectively disables chunking and sharding for it.
...
3. When using no compression, chunk-sizes are constant, and shard indices can be known ahead of time, allowing
   ...
   to a higher shard-size for this dimension. This is beneficial if later access typically constists of blocks with
   multiple slices.
4. Strategy 2 can be combined with compression by either
   * compressing chunks after all slices of a "shard-layer" are written, or
   * directly writing compressed chunks, but assuming a constant chunk-size for the index,
     which results in unused space. This can be compacted after all slices of a
     "shard-layer" are written.
...
* When updating compressed chunks one might also update the chunk in-place if the following
index provides enough space, but the index must still be updated if the byte-length of the chunk changed.

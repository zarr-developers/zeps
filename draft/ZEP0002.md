...
strategies. However, sharding is not meant to distribute loads in this case,
since all shards will mainly be served via the same underlying storage, but it
instead reduces the number of stored files or objects.

When comparing strategies 3 and 2, it's important to note that it is less
important to do load balancing than to co-locate data that is usually accessed
together or in short succession. The latter can be achieved by combining
multiple chunks that are close to each other, which is supported best by
strategy 2, using regular n-dimensional blocks of chunks as sharding units.

...
description: This ZEP proposes to add a sharding storage transformer to
  the Zarr specification version 3. This allows to combine multiple chunks
  into storage keys, enabling the usage of larger arrays with Zarr.
parent: draft ZEPs
nav_order: 1
---
...
This ZEP proposes to add a sharding storage transformer to
the Zarr specification version 3. This allows to combine multiple chunks
into storage keys, enabling the usage of larger arrays with Zarr.
...
For example, the file block size and maximum inode number restrict the usage of numerous small files
...
When comparing strategies 3 and 2, it's important to note that it is less important to do load balancing than to co-locate
...
chunking and indexing are done. Therefore it could be implemented directly in the storage layer,

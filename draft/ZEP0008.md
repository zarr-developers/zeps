...
- `http:` and `https:` for generic HTTP servers
  ...
  In general only single-key read operations (corresponding to GET
  requests) are supported.

  Implementations may choose to heuristically support list operations
  by detecting server support for HTML directory listings and/or
...
pipeline.  The initial `absolute_path` or `relative_path` applies to the path

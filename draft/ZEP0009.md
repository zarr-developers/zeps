...
<!-- originally developed under https://hackmd.io/JeDQnykrSliFx3EQ-D64LA -->
...
## Phase 1: Emergency clarifications
...
<!-- Removing. Ok Norman?
- Remove an existing key that was required by previous versions of the extension.
- Change an existing key to have a value that is invalid under a previous version of the extension.
-->
...
<!-- Replaced by previous section
### Extension "wrappers"


A downside of the `must_understand` strategy within the specification is that
implementations are forced to check on multiple styles of extension. An
addition to the specification could formalize the `name/configuration`
"wrapper" specified above so that implementations could write code that would
work with both of these forms, equally:

```javascript
{
  "https://example.com/offset": [12]
}
```
or
```javascript
...
where all identifiers are now prefixed with a URI full scoping the value. The
...
<!--
TODO (Josh): I suggest including these in the description of the opened PR.

## Future work

- Formalize "store" specifications
- Make a ZEP for `zstd` and get votes
- Fix the ZEP voting process
    - Make sure the ZSC is engaged
    - 6 week voting period for the ZIC. Reserve the right to remove ZIC members in case of no vote.
- Further ZEPs
    - Deprecating `storage_transformers`
    - Deprecating `r*` data types
    - Prefix-based names
        With **prefix-based names**, the prefixes are shortcuts for a URI-prefix and belong to a maintainer (either a team or a single user). The maintainer can create any extension they want under that prefixed name space. The special character `.` (or `:` tbd) is chosen to delimit prefixes. It is expected that the maintainer create a versioning and compatibility policy, but there are no guarantees. The prefix assignment is managed through a `zarr-developers`-owned git repo. Prefixes are never unassigned or reassigned. Example: `imagecodecs.jpeg2000` or `imagecodecs:jpeg2000`
...
### 2025-01-20
- Remove `version` key
- Make promoting of "core extensions" explicit
- Rephrase problem statement
- Remove prefix-based names
- Clarifying which parts require a ZEP and which do not

// - Split rollback into "clarifications" and needs-ZEP parts
//   - 0: Naming (raw & URI-based), regex for raw `[a-z0-9-_.]+` and URI `http(s)://[a-z0-9.-_]+.` -> disallow prefix
//   - 1: Extensions block
//   - 2: Prefixes
//   - 3: Other extension points
// - Move json-ld to future work

// - Example: offset extension
// - Example: OME
// - Example:
// - Set up Zarr-specs team to manage raw names
